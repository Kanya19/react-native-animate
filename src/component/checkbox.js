...
class Checkbox extends React.Component {
  ...
  render() {
    const {size, check, disabled, style, onPress, color} = this.props
    const checkSvg = `
    <svg width="16px" height="16px" viewBox="0 0 16 16" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        <title>Untitled</title>
        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
            <g id="checked" transform="translate(0.875000, 0.875000)" fill="${color || '#E73A32'}" fill-rule="nonzero">
        ...
        <View style={style}>
          {check ? (
            <FastImage
              ...
              source={require('~/asset/img/app/9clci1_check.png')}
            />
          ) : (
            // <SvgXml xml={checkSvg} width={size} height={size} />
            <View
              style={{
                width: size,
                height: size,
